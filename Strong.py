...
## here the file names, you can easyly change and adopt to code,
...
with open(graph_csv, 'r') as csvfile:
    ...
    for row in readCSV:
        ...
        j = 0
        i = i + 1
sub.lower_bound=g.KruskalMST(sub)
sub.best_lower_bound = sub.lower_bound;
#sub.best_lower_bound_array.append(sub.lower_bound);
print("after first kruskal")
######################################################################
"""
...
with open(graph_csv, 'r') as csvfile:
    ...
    for row in readCSV:
        ...
        j = 0
        i = i + 1
print("sub.graph_example_array",sub.graph_example_array)
z = 0
...
#print("asddsaasddsa",total_cost_array_copy_1)
...
print("conf", conf_arr_once)
print("conf array", sub.conflic_arr_just_true)

m = 0
lambda_ef = []
for edge in conf_arr_once:
    lambda_ef.append([])
    count = sub.conflic_arr_just_true[0].count(edge)
    for _ in range(count):
        lambda_ef[m].append(0)
    m += 1
...
while(True):   #until too many iterations
    ...
    ## line 9 update Ce ##
    print("total cost array: ", sub.total_cost_array)
    ...
    for edge in range(len(sub.total_edge_array)):
        if(sub.total_edge_array[edge] in conf_arr_once):
            ...
            line_count = len(lambda_ef[line])
            for i in range(line_count):
                sub.total_cost_array[edge] += 2*lambda_ef[line][i]
    print("lamda ef :: ", lambda_ef)
    ...
    for edge in sub.total_edge_array:
        ...
        i+=1
    print("graph example array:", sub.graph_example_array)


    for column in range(len(sub.graph_example_array)):
        for row in range(len(sub.graph_example_array[column])):
            if(column<row and sub.graph_example_array[column][row] != 0):
                #print("column: ",column, "row: ",row, "weight: ", sub.graph_example_array[column][row])
                g.addEdge(column, row,sub.graph_example_array[column][row])


    z1=g.KruskalMST(sub)
    z = z1 - sum_of_all_lambda_ef
    print("sum of all lambda ef :" , sum_of_all_lambda_ef)
    sum_of_all_lambda_ef = 0


    sub.sum_g = 0
    flag_for_g = 0
    #print("conf arr just true : ",sub.conflic_arr_just_true)

    #Sum G^2 Bulma
    for conflict in range(len(sub.conflic_arr_just_true[0])):
        g1=sub.conflic_arr_just_true[0][conflict]  ##it can be more than 2 conflict solve later
        g2=sub.conflic_arr_just_true[1][conflict]
        ...
        sub.sum_g += (1-g1_value-g2_value)*(1-g1_value-g2_value)
    ...
    #print("conflict_counter_for_kruskal_edges",conflict_counter_for_kruskal_edges)
    ...
    print("conflict: ", conflict_counter_for_kruskal_edges)
    ...
    for i in range(max+1) :
        for j in range(len(conflict_counter_for_kruskal_edges)):
            ...
            if(i == conflict_counter_for_kruskal_edges[j] and sub.edge_array_kruskal[j] not in g_for_upper_bound.changed_edges):
                #print("conf counter : ", sub.edge_array_kruskal[j])
                #print("i",i)
                #print("changed : ", changed_edges)
                if(i == 0):
    ...
    upper = 0
        # print("before : ",total_cost_array_copy_1)
    ...
    print(sub.edge_array_kruskal_for_upper_bound)
    ...
    #Lambda_ef güncelleme
    counter = []
    for i in conf_arr_once:
        counter.append(0)
    ## determine the subgradiendts ##
    for conflict in range(len(sub.conflic_arr_just_true[0])):
        # print("conflict ",conflict)
        g1 = sub.conflic_arr_just_true[0][conflict]
        g2 = sub.conflic_arr_just_true[1][conflict]
        if (g1 in edge_array):  # kruskal
            g1_value = 1
        else:
            g1_value = 0
        ...
        index = conf_arr_once.index(sub.conflic_arr_just_true[0][conflict])
        temp_last = -1 + g1_value + g2_value
        # temp_last = g1_value+g2_value-1
        # print(temp_last)
        # temp_last=max_func(0,temp_last)  #not mantioned on psuedo code
        lambda_ef[index][counter[index]] += sub.a * temp_last
        lambda_ef[index][counter[index]] = max_func(0, lambda_ef[index][counter[index]])
        sum_of_all_lambda_ef += lambda_ef[index][counter[index]]
